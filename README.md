...
```
I would like to propose to replace/update x with the smaller and faster y.
...
y currently pulls in {number} dependencies while x only uses {number} dependencies.
Using y instead of x would reduce the package size by {number} bytes and traffic generated by the package by {number} bytes.

If you welcome this change, i would be happy to provide a PR!
```
